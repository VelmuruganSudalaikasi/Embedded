...
void Atoi(int);
unsigned char array[10],plus='+',minus='-',equals='=',zero='0';
...
void main(){
    ...
    Lcd_displaydata(minus);

    Atoi(Secondnumber);
    result=Firstnumber-Secondnumber;
    Lcd_displaydata('=');
    if(result>0){ Lcd_displaydata(plus);}
...
void Atoi(int number){
    int temp=number;
    i=0;
    while(temp){
        number=temp%10;
        array[i++]=(unsigned char)number+48;
        temp/=10;

    }i--;
    WriteFunction(i);
}
void WriteFunction(int j){
    for(;j>=0;j--)
